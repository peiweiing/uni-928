...
<script type="text/javascript">
	...
	var __url__ = '/yyw/index.php/Home/Attention';
</script>
...
<div class="user_main">
  ...
  <div class="main_right">
    <div class="div_"> 个人中心 &gt;&gt; 我的关注 </div>

    <!--TAB BEGIN-->
    <div class="tab_list">
	  <div class="tab_nv">
	    <ul>
		  <li class="tab_item current"><a href="/yyw/index.php/Home/Attention/index">我的关注</a></li>
		  <li class="tab_item"><a href="/yyw/index.php/Home/ByAttention/index">我的粉丝</a></li>
		  <li class="tab_item"><a href="/yyw/index.php/Home/BySee/index">谁看过我</a></li>
		  <li class="tab_item"><a href="/yyw/index.php/Home/See/index">我看过谁</a></li>
    ...
    <div class="div_smallnav_content_hover" style='width:780px;border:1px solid #ddd' >
	  	  	  <table class="user-table table-margin lh35" border="0" cellpadding="0" cellspacing="0" width="98%">
		<tbody><tr>
		
		 
		 <?php if($users == null): ?><td align="center" style="color:red" id='meiyou'><b>对不起，你还没关注任何人！</b></td>
		 <?php else: ?>
		 <?php if(is_array($users)): foreach($users as $k=>$vo): ?><div class="myfollow_item" id="xx<?php echo ($res["$k"]["attentionid"]); ?>" style="height:100px">
		 
		  
			  <!--$vo.avatar头像地址-->
			<div class="myfollow_logo"><a href="/yyw/index.php/Home/HomePage/index/id/<?php echo ($vo["id"]); ?>" target="_blank"><img class="user-logo" src="/yyw/Public/Uploads/pic/<?php echo ($vo["avatar"]); ?>" width="50" height="60" /></a></div>
			<ul class="myfollow_info">
			  <li><a href="/yyw/index.php/Home/HomePage/index/id/<?php echo ($vo["id"]); ?>" target="_blank"><?php echo ($vo["username"]); ?></a>&nbsp;&nbsp;<?php echo ($res["$k"]["ageyear"]); ?>岁</li>
			  <li> 
			  <?php echo ($res["$k"]["marrystatus"]); ?> <?php echo ($res["$k"]["height"]); ?>CM&nbsp;&nbsp;<?php echo ($city1["$k"]["name"]); ?>&nbsp;<?php echo ($city["$k"]["name"]); ?>
			  </li>
			  <li>
			  <img src="/yyw/Public/images/cid_h.png" title="身份证未认证" />
			  <img src="/yyw/Public/images/video_h.png" title="视频未认证" />
			  <img src="/yyw/Public/images/email_h.png" title="邮箱未认证" /> 
			  </li>
			</ul>
			<div class="clear"></div>
			<div class="myfollow_db" id="myfollow_184625_db">
					  <font color='#999999' title='<?php echo ($res["$k"]["monolog"]); ?>'><?php echo ($res["$k"]["monolog1"]); ?></font>
					
			</div>
			<div style='color:#ddd;display:none' class='mouse'>
			<a href="javascript:void(0)" onclick="showBox('greet',<?php echo ($vo["id"]); ?>);">打招呼</a> | 
			<a href="javascript:void(0)" onclick="showBox('message',<?php echo ($vo["id"]); ?>);">写信件</a> | 
			<a href="###" onclick="return doClick(<?php echo ($res["$k"]["attentionid"]); ?>);">删除</a>
			</div>	
	</div>
...
<script type='text/javascript'>
	...
	$(".myfollow_item").mouseover(function(){
		$(this).children('.mouse').css('display','block');
	});
	$(".myfollow_item").mouseout(function(){
		$(this).children('.mouse').css('display','none');
	});
	
	function doClick(attentionid){
	var otherid = $('#dongtai').attr('value');
			$.ajax({
			url:'/yyw/index.php/Home/Attention/del',
			type:'post',
			dataType:'json',
			data:{ id:attentionid },
			success:function(info){
				if(info['error'] == true){
					$('#xx'+attentionid).fadeOut("slow");
				}else{
					alert(info['data']);
				}
			}
