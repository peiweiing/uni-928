...
class PhwbController extends CommonController
{
	protected function _tigger_list(&$list){
		foreach($list as &$v){
			$v['userid'] = D('user')->where('id='.$v['uid'])->getField('username');
		}
	}
	...
	protected function _list($model, $map = array(), $sortBy = '', $asc = false) {
		...
		//分页查询数据
		$list = $model->field('id,content,time,uid')->where($map)->order($order.' '.$sort)
